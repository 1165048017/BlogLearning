...
%         % Visualize weights
%         figure(1);
%         W1 = reshape(x(1:options.hiddenSize * 784), options.hiddenSize, 784);
%         display_network(W1');
%     end
%     
%     if options.displayImage2
%         % Visualize weights
%         figure(1);
%         W2 = reshape(x((options.hiddenSize2*10)+1 : options.hiddenSize2 * 10 + options.hiddenSize1 * 784), options.hiddenSize1, 784);
%         display_network(W2');
% 
%         softmaxTheta = reshape(x(1:options.hiddenSize2*options.classes), options.classes, options.hiddenSize2);
%         stack = params2stack(x(options.hiddenSize2*options.classes+1:end), options.config);
%         
%         a = cell(numel(stack)+2,1);
%         a{1} = options.data;
%         for ii = 2:numel(a)-1
%             a{ii} = sigmoid(stack{ii-1}.w*a{ii-1} + repmat(stack{ii-1}.b,1,size(a{ii-1},2)));
%         end
